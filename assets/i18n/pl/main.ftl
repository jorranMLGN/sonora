...
login-problem-region = { $provider } nie otworzy sesji z kraju, w którym jesteś. Zaloguj się z kraju macierzystego albo zmień kraj na koncie { $provider }.
login-problem-credentials = Zapisana sesja { $provider } straciła ważność. Zaloguj się ponownie.
login-problem-network = Sonora nie mogła połączyć się z serwisem { $provider }. Sprawdź połączenie internetowe i spróbuj ponownie.
login-problem-cancelled = Zamknięto stronę przeglądarki przed zatwierdzeniem logowania. Zacznij od nowa.
login-problem-refused = { $provider } odrzucił logowanie. Odczekaj chwilę i spróbuj ponownie.
