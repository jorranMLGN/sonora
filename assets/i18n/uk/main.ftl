...
login-problem-region = { $provider } не відкриває сесію з країни, де ви перебуваєте. Увійдіть з домашньої країни або змініть країну в акаунті { $provider }.
login-problem-credentials = Збережена сесія { $provider } більше недійсна. Увійдіть знову.
login-problem-network = Sonora не змогла зв'язатися зі службою { $provider }. Перевірте інтернет-з'єднання та спробуйте ще раз.
login-problem-cancelled = Ви закрили сторінку браузера, не підтвердивши вхід. Почніть знову.
login-problem-refused = { $provider } відхилив вхід. Зачекайте трохи і спробуйте ще раз.
