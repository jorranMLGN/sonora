...
login-problem-region = { $provider } n'ouvre pas de session depuis le pays où vous êtes. Connectez-vous depuis votre pays d'origine, ou changez le pays de votre compte { $provider }.
login-problem-credentials = Votre session { $provider } enregistrée n'est plus valide. Reconnectez-vous pour continuer.
login-problem-network = Sonora n'a pas pu joindre { $provider }. Vérifiez votre connexion internet et réessayez.
...
login-problem-refused = { $provider } a refusé la connexion. Patientez un instant et réessayez.
