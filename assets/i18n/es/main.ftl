...
login-problem-region = { $provider } no abre una sesión desde el país en el que estás. Inicia sesión desde tu país de origen o cambia el país en tu cuenta de { $provider }.
login-problem-credentials = Tu sesión guardada de { $provider } ya no es válida. Inicia sesión de nuevo para continuar.
login-problem-network = Sonora no pudo conectarse con { $provider }. Revisa tu conexión a internet e inténtalo de nuevo.
...
login-problem-refused = { $provider } rechazó el inicio de sesión. Espera un momento e inténtalo de nuevo.
