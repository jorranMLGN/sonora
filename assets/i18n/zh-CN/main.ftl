...
login-problem-region = { $provider } 不会从你所在的国家/地区开启会话。请从你的所在国家/地区登录，或更改 { $provider } 账户上的国家/地区。
login-problem-credentials = 你保存的 { $provider } 会话已失效。请重新登录以继续。
login-problem-network = Sonora 无法连接到 { $provider }。请检查网络连接并重试。
login-problem-cancelled = 你在批准登录之前关闭了浏览器页面。请重新开始以完成登录。
login-problem-refused = { $provider } 拒绝了登录请求。请稍等片刻后重试。
