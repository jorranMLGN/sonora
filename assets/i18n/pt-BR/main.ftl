...
login-problem-region = O { $provider } não abrirá uma sessão do país onde você está. Faça login do seu país de origem ou altere o país na sua conta do { $provider }.
login-problem-credentials = Sua sessão salva do { $provider } não é mais válida. Faça login novamente para continuar.
login-problem-network = O Sonora não conseguiu acessar o { $provider }. Verifique sua conexão com a internet e tente novamente.
login-problem-cancelled = Você fechou a página do navegador antes de aprovar o login. Recomece para finalizar.
login-problem-refused = O { $provider } recusou o login. Aguarde um momento e tente novamente.
