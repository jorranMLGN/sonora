...
login-problem-region = { $provider } öffnet keine Sitzung aus dem Land, in dem du dich befindest. Melde dich aus deinem Heimatland an oder ändere das Land in deinem { $provider }-Konto.
login-problem-credentials = Deine gespeicherte { $provider }-Sitzung ist nicht mehr gültig. Melde dich erneut an, um fortzufahren.
login-problem-network = Sonora konnte { $provider } nicht erreichen. Prüfe deine Internetverbindung und versuche es erneut.
...
login-problem-refused = { $provider } hat die Anmeldung abgelehnt. Warte einen Moment und versuche es erneut.
