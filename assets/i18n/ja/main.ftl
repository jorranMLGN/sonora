...
login-problem-region = 現在いる国からは { $provider } のセッションを開始できません。居住国からサインインするか、{ $provider } アカウントの国設定を変更してください。
login-problem-credentials = 保存された { $provider } のセッションは無効になっています。続けるにはもう一度サインインしてください。
login-problem-network = { $provider } に接続できませんでした。インターネット接続を確認してもう一度お試しください。
login-problem-cancelled = サインインを承認する前にブラウザのページが閉じられました。最初からやり直してください。
login-problem-refused = { $provider } がサインインを拒否しました。しばらく待ってからもう一度お試しください。
