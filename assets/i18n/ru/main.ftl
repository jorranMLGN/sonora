...
login-problem-region = { $provider } не открывает сессию из страны, где вы находитесь. Войдите из домашней страны или смените страну в аккаунте { $provider }.
login-problem-credentials = Сохранённая сессия { $provider } больше недействительна. Войдите заново.
login-problem-network = Sonora не смогла связаться со службой { $provider }. Проверьте интернет-соединение и попробуйте снова.
login-problem-cancelled = Вы закрыли страницу браузера, не подтвердив вход. Начните заново.
login-problem-refused = { $provider } отклонил вход. Подождите немного и попробуйте снова.
