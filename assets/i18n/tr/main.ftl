...
login-problem-region = { $provider }, bulunduğun ülkeden oturum açılmasına izin vermiyor. Kendi ülkenden oturum aç ya da { $provider } hesabındaki ülkeyi değiştir.
login-problem-credentials = Kayıtlı { $provider } oturumun artık geçerli değil. Sürdürmek için yeniden oturum aç.
login-problem-network = Sonora, { $provider }'a ulaşamadı. İnternet bağlantını denetleyip yeniden dene.
login-problem-cancelled = Oturum açmayı onaylamadan tarayıcı sayfasını kapattın. Bitirmek için yeniden başla.
login-problem-refused = { $provider } oturum açma isteğini geri çevirdi. Biraz bekleyip yeniden dene.
