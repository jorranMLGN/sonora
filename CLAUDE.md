...
### Jam: playing to the local network

`state::jam` broadcasts what is playing to any device on the same network, in sync to the
millisecond. `server.rs` serves `page.html` and `/stream.wav` over a small HTTP and websocket
listener on `settings.json`'s `jam.port` (8990 by default, `SONORA_JAM_PORT` overrides);
`receiver.rs` is the native listener and `cast.rs` the sink that taps the engine's output after
the equalizer.

Sync is NTP-shaped and lives in `clock.rs`: four timestamps per probe, `offset` from the mean and
`rtt` from the round trip, with the lowest-RTT sample of a burst kept. A listener corrects in one
of three regimes — hold, slew, anchor — depending on how far off it is, so a small drift is eased
away by bending playback rate rather than by a jump. `lead.rs` negotiates the buffer the whole
room shares: every listener reports what it needs, the host takes the maximum, raises at once and
lowers only after it has held for `HOLD`.

`wire.rs` is the protocol, versioned by `PROTOCOL`; bump it whenever a message changes shape.
`snapshot.rs` builds what each device is allowed to see — `between()` sends only what changed,
and a device's capabilities follow the grant the host gave that seat, so revoking one reaches
that device alone.

The browser receiver is one page with no build step. It schedules audio against
`origin + first_sample / rate + lead` read from `AudioContext.getOutputTimestamp()`, and sets
`navigator.audioSession.type = "playback"` so iOS does not silence it with the ringer switch.
...
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Session`                                  | auth lifecycle; emits `SessionEvent::{SignedIn, SignedOut, Reconnected}` with the provider's slug, plus `LocalChanged`; hands out `Arc<dyn MusicApi>` and `Arc<dyn PlaybackFactory>` per slug                                                  |
...
**One shelf per provider, one code path.** Several providers are signed in at once, so `state::Shelf`
names one of them: `Shelf::Of(slug)` is a provider by slug, `Shelf::Local` the imported files, and
`Shelf::Streaming` whichever provider the app is showing. `Shelf::of(id)` routes an id to its owner
through its tag. `Library` holds one `Held` per slug with the same loading, landing and favorites
code, and every accessor takes the shelf: `state(shelf)`, `loading(shelf, part)`,
`part_failed(shelf, part)`, `shape(shelf)`. `Session` answers `client_of(shelf)`, `shape_of(shelf)`
and `capabilities_of(shelf)`. Do not add a `local_*` twin of anything; branch on `Shelf` or on
`Shape`.

**Ids carry their provider.** `music::tagged` wraps any `MusicApi` and prefixes every id it hands
out with the provider's slug, so `music::tag::slug_of(id)` answers who owns a track wherever it
turns up — the queue, the player, a pinned card. `music::tag::untag` strips it back off at the
provider boundary. A new provider needs no routing code of its own; it is registered in
`main.rs` and everything else follows the tag.

**A library route names its provider.** `Destination::Library(slug, LibraryTab)` is the route, and
`Root` keeps one `LibraryView` per slug. The sidebar lists one expandable group per signed-in
provider (`sidebar_left::Group::Provider(slug)`), plus Settings, plus the pinned section.

**Local Music is one more provider.** Its slug is `local`, so `Destination::Library(LOCAL, tab)`
reaches it through the same route as any other shelf and `Root` builds it the same `LibraryView`.
Every shelf has the same four tabs, Songs included, whatever the shape.
