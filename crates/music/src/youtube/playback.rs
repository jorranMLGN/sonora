...
impl PlaybackFactory for Factory {
    fn start(
        ...
        let spawned = std::thread::Builder::new()
            ...
            .spawn(move || run(api, config, command_rx, events, engine_spectrum, cast));
...
fn run(
    ...
    runtime.block_on(engine_loop(api, config, commands, events, spectrum, cast));
}
...
async fn engine_loop(
    ...
    let output = match Output::open(Volume::new(config.gain), spectrum, "youtube", cast) {
        Ok(output) => output,
        Err(error) => {
