...
pub enum Shelf {
    /// Whichever streaming provider the app is showing right now.
    Streaming,
    Local,
    /// One signed-in provider by slug. Several are live at once, so anything that routes by id
    /// names the provider rather than the shelf it happens to be drawn on.
    Of(&'static str),
}

impl Shelf {
    /// The shelf an id belongs to. Local files have their own; everything else belongs to the
    /// provider its tag names, so two signed-in providers never read each other's rows.
    pub fn of(id: &str) -> Self {
        match whose(id) {
            Some(music::tag::LOCAL) => Self::Local,
            Some(slug) => Self::Of(slug),
            None => Self::Streaming,
        }
    }

    pub fn local(self) -> bool {
        matches!(self, Self::Local) || matches!(self, Self::Of(slug) if slug == music::tag::LOCAL)
    }
...
pub struct Library {
    shelves: HashMap<&'static str, Held>,
    /// The provider whose library the app is showing. `Shelf::Streaming` follows it.
    active: Option<&'static str>,
    /// Answered for a shelf that has no provider, so a reader never has to hold an option.
    empty: Held,
    ...
    snapshots: Snapshots,
    priming: HashMap<&'static str, Task<()>>,
}
...
impl Library {
    pub fn new(
        ...
        cx.subscribe(&session, |this, session, event, cx| match event {
            SessionEvent::SignedIn(slug) => {
                let shelf = Shelf::Of(slug);
                this.active = session.read(cx).provider_slug();
                if slug != &music::tag::LOCAL {
            ...
            SessionEvent::SignedOut(slug) => {
                ...
                this.priming.remove(*slug);
                this.pending.retain(|id, _| whose(id) != Some(*slug));
                this.pending_albums.retain(|id, _| whose(id) != Some(*slug));
                this.pending_artists
                    .retain(|id, _| whose(id) != Some(*slug));
                this.pending_library
                    .retain(|id, _| whose(id) != Some(*slug));
                this.contents.retain(|id, _| whose(id) != Some(*slug));
                this.reading.retain(|id, _| whose(id) != Some(*slug));
                this.mosaics.retain(|id, _| whose(id) != Some(*slug));
                if slug != &music::tag::LOCAL {
                    this.sidebar_pin_task = None;
        ...
        library.active = library.session.read(cx).provider_slug();
        for shelf in library.shelves(cx) {
            library.held_mut(shelf).state = LibraryState::Loading;
            library.prime(shelf, cx);
            library.load(shelf, cx);
        }
    ...
    fn prime(&mut self, shelf: Shelf, cx: &mut Context<Self>) {
        ...
        let provider = match shelf {
            ...
            Shelf::Of(slug) if slug == music::tag::LOCAL => {
                (!session.local_paths().is_empty()).then(|| session.local_slug())
            }
            Shelf::Of(slug) => Some(slug),
        };
        ...
        self.priming.insert(primed, task);
    }
    ...
    fn forget(&mut self, shelf: Shelf, cx: &App) {
        ...
        if let Some(slug) = self.slug_of(shelf) {
            self.priming.remove(slug);
        }
        let snapshots = self.snapshots.clone();
    ...
    /// The slug a shelf stands for. `Streaming` follows whichever provider the app is showing.
    fn slug_of(&self, shelf: Shelf) -> Option<&'static str> {
        match shelf {
            Shelf::Streaming => self.active,
            Shelf::Local => Some(music::tag::LOCAL),
            Shelf::Of(slug) => Some(slug),
        }
    }
    ...
    fn held(&self, shelf: Shelf) -> &Held {
        self.slug_of(shelf)
            .and_then(|slug| self.shelves.get(slug))
            .unwrap_or(&self.empty)
    }

    fn held_mut(&mut self, shelf: Shelf) -> &mut Held {
        let Some(slug) = self.slug_of(shelf) else {
            return &mut self.empty;
        };
        self.shelves.entry(slug).or_insert_with(Held::empty)
    }

    /// Every signed-in provider's shelf, in provider order.
    fn shelves(&self, cx: &App) -> Vec<Shelf> {
        self.session
            .read(cx)
            .active_slugs()
            .into_iter()
    ...
    fn adopt_mosaics(&mut self, slug: &'static str) -> Vec<(String, u32)> {
        let Some(ready) = self.held_mut(Shelf::Of(slug)).ready_mut() else {
            return Vec::new();
        };
