...
/// The navigation entries that expand into tabs rather than navigate: one per signed-in
/// provider, plus Settings.
...
impl Group {
    fn of(destination: &Destination) -> Option<Self> {
        match destination {
            Destination::Library(slug, _) => Some(Self::Provider(slug)),
            Destination::Settings(_) => Some(Self::Settings),
            _ => None,
        }
    }

    /// What the open set files the group under.
    fn id(self) -> &'static str {
        match self {
            Self::Provider(slug) => slug,
            Self::Settings => "settings",
        }
    }
}

/// One navigation row per signed-in provider, in provider order, with History and Settings
/// around them.
fn nav(session: &Session) -> Vec<(Option<NavEntry>, &'static str, Destination)> {
    ...
    nav.extend(libraries(session).map(|slug| {
        (
            ...
            provider_logo(slug),
            Destination::Library(slug, LibraryTab::Songs),
        )
    }));
    nav.extend([
        (
...
impl SidebarLeft {
    pub fn new(cx: &mut Context<Self>) -> Self {
        ...
        let groups = expanded(&at).into_iter().map(Group::id).collect();
    ...
    fn follow(&mut self, current: &Destination) {
        ...
        self.groups.extend(expanded(current).map(Group::id));
    }
    ...
    fn navigation(&self, cx: &mut Context<Self>) -> Vec<AnyElement> {
        let entries = nav(self.session.read(cx));
        let mut rows = Vec::new();
        for (index, (entry, _, destination)) in entries.iter().enumerate() {
            if entry.is_some_and(|entry| !self.settings.read(cx).nav_shown(entry.id())) {
                continue;
            }
            // A shelf with rows on it belongs in the sidebar even before the session has
            // restored, since a snapshot fills it while the provider is still answering.
            if let Destination::Library(slug, _) = destination
                && *slug != LOCAL
                && !self.session.read(cx).authenticated_for(slug)
                && !self.library.read(cx).stocked(Shelf::Of(slug))
            {
                continue;
            }
    ...
    fn flip(&mut self, group: Group) {
        if !self.groups.remove(group.id()) {
    ...
    fn nav(&self, index: usize, cx: &mut Context<Self>) -> AnyElement {
        let theme = *cx.theme();
        let accent = theme.sidebar_accent;
        let (entry, icon, destination) = nav(self.session.read(cx))[index].clone();
        ...
        let row = nav_row(index, label, tint, accent).icon(icon);
    ...
    fn tabs(&self, group: Group, cx: &mut Context<Self>) -> AnyElement {
        ...
        let tab = |id: ElementId, key: &'static str, destination: Destination| {
            ...
            nav_row(id, i18n::lookup(key, None), tint, accent)
                .flex_1()
        ...
        match group {
            Group::Provider(slug) => Tabs::new().items(LibraryTab::ALL.into_iter().map(|tab_id| {
                tab(
                    (ElementId::from(slug), tab_id.key()).into(),
                    tab_id.key(),
                    Destination::Library(slug, tab_id),
                )
            })),
...
/// The group a route opens, if it is inside one.
fn expanded(current: &Destination) -> Option<Group> {
    Group::of(current)
}
...
mod tests {
    ...
    use super::{Group, expanded};

    #[test]
    fn a_section_expands_only_where_it_leads() {
        assert!(matches!(
            expanded(&Destination::Library("spotify", LibraryTab::Albums)),
            Some(Group::Provider("spotify"))
        ));
        assert!(matches!(
            expanded(&Destination::Library(router::LOCAL, LibraryTab::Albums)),
            Some(Group::Provider(router::LOCAL))
        ));
        assert!(matches!(
    ...
    #[test]
    fn content_belongs_to_no_section() {
        let away = [
            Destination::Home,
            Destination::Search,
        ...
        for destination in away {
            assert!(expanded(&destination).is_none(), "{destination:?}");
        }
    }
}
