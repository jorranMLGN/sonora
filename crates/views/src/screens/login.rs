...
impl LoginView {
    ...
    fn option_buttons(
        ...
        let options = match method {
            ...
            SignIn::Browser(_) => vec![LoginOption {
                id: format!("sign-in-{slug}-browser").into(),
                label: t!("login-import-browser"),
                action: LoginAction::SignIn(method.clone()),
                primary: false,
            }],
            SignIn::Default | SignIn::Anonymous | SignIn::Path(_) | SignIn::Credentials { .. } => {
                let (suffix, label, action, primary) = match method {
                    ...
                    SignIn::Secret | SignIn::Browser(_) => unreachable!(),
                };
                vec![LoginOption {
